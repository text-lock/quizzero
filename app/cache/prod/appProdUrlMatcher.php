    ...
    public function match($pathinfo)
    {
        ...
        if (0 === strpos($pathinfo, '/admin/qu')) {
            if (0 === strpos($pathinfo, '/admin/questions')) {
                ...
                // questionlink_delete
                if (preg_match('#^/admin/questions/(?P<id>[^/]++)/delete$#s', $pathinfo, $matches)) {
            ...
            if (0 === strpos($pathinfo, '/admin/quiz')) {
                ...
                if (preg_match('#^/admin/quiz/(?P<id>[^/]++)/edit$#s', $pathinfo, $matches)) {
                    ...
                    return $this->mergeDefaults(array_replace($matches, array('_route' => 'quizlink_edit')), array (  'question' => NULL,  '_controller' => 'Quizzero\\QuizBundle\\Controller\\QuizController::editAction',));
                }
                not_quizlink_edit:

                // quizlink_delete
                if (0 === strpos($pathinfo, '/admin/quiz/delete') && preg_match('#^/admin/quiz/delete/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                    if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                        $allow = array_merge($allow, array('GET', 'HEAD'));
        ...
        if (0 === strpos($pathinfo, '/user/result')) {
            // result_new
            if ($pathinfo === '/user/result/new') {
                if (!in_array($this->context->getMethod(), array('GET', 'POST', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'POST', 'HEAD'));
                    goto not_result_new;
                }
            ...
            // result_show
            if (preg_match('#^/user/result/(?P<id>[^/]++)$#s', $pathinfo, $matches)) {
                if (!in_array($this->context->getMethod(), array('GET', 'HEAD'))) {
                    $allow = array_merge($allow, array('GET', 'HEAD'));
        ...
        if (0 === strpos($pathinfo, '/quiz') && preg_match('#^/quiz/(?P<id>\\d+)$#s', $pathinfo, $matches)) {
